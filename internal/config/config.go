...
// it writes to ~/.docloom-cli/config.yaml, creating the directory if necessary.
func Save(c *Global, cfgFile string) error {
	...
	if cfgFile != "" {
		...
		dir := filepath.Join(home, ".docloom")
...
func Load(cfgFile string) (*Global, error) {
	v := viper.New()
	// New prefix for DocLoom
	v.SetEnvPrefix("DOCLOOM")
	v.AutomaticEnv()
	...
	// Resolve projects_dir default: ~/.docloom-cli/projects
	if c.ProjectsDir == "" {
		home, err := os.UserHomeDir()
		if err != nil {
			return nil, fmt.Errorf("resolve home dir: %w", err)
		}
		c.ProjectsDir = filepath.Join(home, ".docloom", "projects")
	}
	return &c, nil
}
