...
var modelsCmd = &cobra.Command{
	...
	Example: `  docloom models show
  docloom models sync --file ./models.json
  docloom models sync --file ./models.json --merge
  docloom models fetch --url https://example.com/models.json
...
func providerURL(name string) string {
	...
	// DOCLOOM_OPENROUTER_CATALOG_URL, DOCLOOM_OPENAI_CATALOG_URL, DOCLOOM_ANTHROPIC_CATALOG_URL
	switch name {
	case "openrouter":
		if v := os.Getenv("DOCLOOM_OPENROUTER_CATALOG_URL"); v != "" {
			return v
		}
		// default maintained endpoint
		return "https://raw.githubusercontent.com/KaramelBytes/docloom-cli/main/docs/openrouter-models.json"
	case "openai":
		if v := os.Getenv("DOCLOOM_OPENAI_CATALOG_URL"); v != "" {
			return v
		}
		return ""
	case "anthropic":
		if v := os.Getenv("DOCLOOM_ANTHROPIC_CATALOG_URL"); v != "" {
			return v
		}
		return ""
	default:
