...
var rootCmd = &cobra.Command{
	Use:   "docloom",
	Short: "DocLoom CLI: merge multiple docs into one AI-ready context",
	Long:  `DocLoom is a CLI tool that combines multiple documents into a unified context and sends them to AI models via OpenRouter for analysis and generation.`,
}
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is ~/.docloom-cli/config.yaml)")
...
func loadConfig() {
	...
	if cfg.ModelsAutoSync {
		...
		if url == "" && cfg.ModelsProvider != "" {
			...
			case "openrouter":
				url = "https://raw.githubusercontent.com/KaramelBytes/docloom-cli/main/docs/openrouter-models.json"
			}
		}
		if url != "" {
